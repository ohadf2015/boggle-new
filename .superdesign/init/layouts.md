...
The main layout component for Blast Mode. Portrait-oriented, single-column layout with:
- Header (quit button, wave badge, help button, end game button)
- Combo display
- Combo milestone announcements (NICE!, FIRE!, MYTHIC!, GODLIKE!)
- Cascade word showcase banners
- Stats row (score, words count, progress bar)
- Score threshold indicator (wave 3+)
- Cumulative score indicator (wave 2+)
- Word forming area (LetterTileWord)
- Found words expandable list
- Dead-end notification with shuffle/end game buttons
- Game grid with overlays (BlastGrid)
- Board complete celebration overlay
- Quit/End Game confirmation dialogs
- Help modal

**File:** `fe-next/components/blast/BlastGameLayout.tsx`
...
export function BlastGameLayout({
  ...
  return (
    <div className={cn('relative flex-1 flex flex-col overflow-hidden h-full bg-neo-navy', shakeClass)}>
      ...
      {/* Fallback cascade chain announcement */}
      <AnimatePresence>
      ...
      <div className={cn(
        ...
        <LetterTileWord word={formedWord} feedback={currentFeedback} />
        {formedWord && (
      ...
      <BlastHelpModal open={showHelp} onOpenChange={setShowHelp} t={t} />
    </div>
  );
}
```
...
Page orchestrator for Blast Mode. Manages wave-based phase transitions: `playing` -> `waveTransition` -> `playing` -> ... -> `results`. Handles wave state, cumulative scoring, and navigation.
