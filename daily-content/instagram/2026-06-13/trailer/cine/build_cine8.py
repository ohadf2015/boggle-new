#!/usr/bin/env python3
# v8: smooth boring->explosion MORPH (no hard cut), aligned music, impact on explosion, gag button.
import os, subprocess, shlex, sys
ROOT=os.path.dirname(os.path.abspath(__file__)); os.chdir(ROOT)
os.makedirs("norm8",exist_ok=True); os.makedirs("out",exist_ok=True)
...
OUT   = sys.argv[2] if len(sys.argv)>2 else "out/lexiclash_trailer_v8.mp4"
GRADE = "eq=contrast=1.05:saturation=1.10:gamma=0.98,vignette=PI/4.6,unsharp=5:5:0.4:5:5:0.0"

SHOTS=[
 ("clips/a1_boring.mp4", 0.0, 5.3, 0.62, True,  False, [("overlays/cap1.png",0.5,3.1),("overlays/cap2.png",3.4,5.3)]),
 ("clips/morph.mp4",     0.1, 3.3, 1.0,  False, False, []),
 ("clips/bridge.mp4",    0.6, 2.6, 1.0,  False, False, [("overlays/cap3.png",0.4,2.6)]),
 ("clips/s2_hero.mp4",   0.3, 2.0, 1.0,  False, False, []),
 ("clips/board.mp4",     0.1, 3.6, 1.0,  False, False, [("overlays/cap4.png",0.3,3.6)]),
 ("clips/s3_rival.mp4",  0.2, 2.0, 1.0,  False, False, []),
 ("clips/cry.mp4",       0.1, 2.3, 1.0,  False, False, []),
 ("clips/dance.mp4",     0.1, 2.6, 1.0,  False, False, [("overlays/cap5.png",0.2,2.6)]),
 ("clips/s6_triumph.mp4",0.2, 2.0, 1.0,  False, False, []),
 ("clips/end_beckon.mp4",0.1, 3.8, 1.0,  False, False, [("overlays/cta_ov.png",0.7,3.8)]),
 ("clips/gag.mp4",       0.1, 2.8, 1.0,  False, False, []),
]
# SMOOTH dissolve boring->morph(0.4); seamless morph->bridge & quick gag(0.12); else 0.22
XF=[0.0]+[0.4 if k==1 else (0.12 if k in (2,10) else 0.22) for k in range(1,len(SHOTS))]
VO=[("vo/g1.wav",0.4,1.06),("vo/g3.wav",6.2,1.06),("vo/g4.wav",12.5,1.10),("vo/g6.wav",22.3,1.06)]
...
run(f'ffmpeg -y -loglevel error {inputs} -filter_complex "{";".join(fc)}" -map "[vout]" {ENC} -an out/_v8.mp4')
print(f"video {TOTAL:.2f}s morph@{SWITCH:.2f} gag@{GAG:.2f}")

extra=[("vo/gag_sfx.wav",GAG+0.3,1.2),("assets/impact.wav",SWITCH+1.0,1.2)]
allsfx=VO+extra
...
run(f'ffmpeg -y -loglevel error {ai} -i {shlex.quote(MUSIC)} -filter_complex "{";".join(af+[mixn,music,final])}" -map "[aout]" -ac 2 -ar 44100 out/_a8.m4a')
run(f'ffmpeg -y -loglevel error -i out/_v8.mp4 -i out/_a8.m4a -c:v copy -c:a aac -b:a 192k -shortest -movflags +faststart {shlex.quote(OUT)}')
