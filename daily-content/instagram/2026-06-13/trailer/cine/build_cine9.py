#!/usr/bin/env python3
# v9: smoother transitions + cohesive music (epic confined to transformation). Music swappable via arg1.
import os, subprocess, shlex, sys
ROOT=os.path.dirname(os.path.abspath(__file__)); os.chdir(ROOT)
os.makedirs("norm9",exist_ok=True); os.makedirs("out",exist_ok=True)
W,H,FPS=1080,1920,30; ENC="-c:v libx264 -pix_fmt yuv420p -profile:v high -preset medium -r 30"
MUSIC = sys.argv[1] if len(sys.argv)>1 else "gamemusic/musicA_epicshort.m4a"
OUT   = sys.argv[2] if len(sys.argv)>2 else "out/lexiclash_trailer_v9.mp4"
...
run(f'ffmpeg -y -loglevel error {inputs} -filter_complex "{";".join(fc)}" -map "[vout]" {ENC} -an out/_v9.mp4')
print(f"video {TOTAL:.2f}s morph@{SWITCH:.2f} gag@{GAG:.2f}")

extra=[("vo/gag_sfx.wav",GAG+0.3,1.2),("assets/impact.wav",SWITCH+1.0,1.1)]
allsfx=VO+extra
...
run(f'ffmpeg -y -loglevel error {ai} -i {shlex.quote(MUSIC)} -filter_complex "{";".join(af+[mixn,music,final])}" -map "[aout]" -ac 2 -ar 44100 out/_a9.m4a')
run(f'ffmpeg -y -loglevel error -i out/_v9.mp4 -i out/_a9.m4a -c:v copy -c:a aac -b:a 192k -shortest -movflags +faststart {shlex.quote(OUT)}')
