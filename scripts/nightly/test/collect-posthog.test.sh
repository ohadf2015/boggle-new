...
# Stub curl: branch on the HogQL body to return per-query fixtures.
cat > "$BIN/curl" <<'STUB'
#!/bin/bash
args="$*"
if echo "$args" | grep -q 'exception'; then
  echo '{"columns":["t","c","u"],"results":[["TypeError",412,38],["RangeError",90,12]]}'
...
assert "an exception → 01-triage"             'jq -e ".signals[] | select(.kind==\"error\" and .lane==\"01-triage\" and (.title|test(\"TypeError\")))" "$OUT" >/dev/null'
assert "exception reach carried (users=38)"   '[ "$(jq -r ".signals[] | select(.title|test(\"TypeError\")) | .reach" "$OUT")" = "38" ]'
