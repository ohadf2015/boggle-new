...
**Sentry sweep comes FIRST, dropped-work restores SECOND:** fix the top Sentry / Supabase / PostHog items from the brief before touching any **Restore dropped nightly work** item — even a STALE one. A restore is only worth this lane's remaining time AFTER the sweep. When you do restore, use ONLY `scripts/nightly/restore-salvaged-code.sh <tag>` — it 3-way-merges each file against that night's `baseline sha=` and SKIPS conflicting files (never copy/rsync a backup over the tree by hand: that reverted master's #1034 WordWheelChallenge.tsx three nights running). A dir with a `LANDED` marker is already landed — just append `{"resolve":"<tag>"}` to `docs/nightly/restore-queue.ndjson` and move on. Leave every SKIPPED file alone; fix the ORIGINAL gate error in the merged files (the drop reason names it — recurring causes: hooks after an early return, `ssr:false` in a Server Component), then append the resolve line. Restored code failed a gate once — it must pass tonight's gate like any other change.
