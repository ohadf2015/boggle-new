...
while IFS= read -r entry; do
  ...
  evidence="restore: scripts/nightly/restore-salvaged-code.sh $tag (backup: $backup). Files: $files_head. Drop reason: $reason. After the restored work is fixed + shipping, append {\"resolve\":\"$tag\"} to docs/nightly/restore-queue.ndjson."
  add "$(emit_signal restore dropped_work "$title" dropped_files "$nfiles" "$nfiles" 0.95 01-triage "" "$evidence" M "restore:$tag")"
