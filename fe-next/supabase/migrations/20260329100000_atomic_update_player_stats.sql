-- Atomic profile stats + XP update to prevent deadlocks
-- Combines the two separate writes (stats UPDATE + increment_player_xp RPC)
-- into a single transaction that acquires locks in a deterministic order.

CREATE OR REPLACE FUNCTION update_player_stats_and_xp(
  p_player_id UUID,
  p_stats JSONB,        -- { total_games, total_score, total_words, ... }
  p_xp_amount INTEGER   -- 0 means skip XP increment
)
RETURNS TABLE(
  new_total_xp BIGINT,
...
DECLARE
  ...
  v_set_parts TEXT[];
BEGIN
  -- Lock the row first with FOR UPDATE to prevent deadlocks
  ...
  -- Build dynamic SET clause from stats JSONB
  -- Only update columns that are present in the JSONB
  v_set_parts := ARRAY['updated_at = NOW()'];

  -- Numeric increment fields
  ...
  -- Apply XP increment if requested
  IF p_xp_amount > 0 THEN
    ...
    -- Calculate new level (matches xpManager.ts getXpForLevel())
    v_new_level := 1;
