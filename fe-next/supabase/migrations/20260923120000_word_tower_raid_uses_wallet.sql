-- Word Tower v2: raids move WALLET coins (profiles.total_coins).
--
-- Word Tower coins were folded into the app-wide wallet (estateServer /
-- estateWallet): the estate row's `coins` column is legacy and is swept into
-- the wallet on every load. Without this migration a raid still works but
-- steals nothing (the column is 0) and the attacker's gain reaches the wallet
-- only via that sweep. This version takes the theft from, and pays the gain
-- into, profiles directly, in the same transaction as the plot damage.
--
-- Only coin lines differ from 20260920120000_word_tower_free_revenge.sql.
...
begin
  ...
  -- Coins are the app-wide wallet now (profiles.total_coins), not the estate row.
  select coalesce(total_coins, 0) into v_def_bal from public.profiles where id = p_defender for update;
  v_stolen := least(greatest(p_coins_stolen, 0), coalesce(v_def_bal, 0))::int;
  ...
  if p_blocked then
    ...
    update public.word_tower_estates
      set plots = case when p_slot is null then plots else (
    ...
    update public.profiles
      set total_coins = coalesce(total_coins, 0) - v_stolen, updated_at = now()
      where id = p_defender;
  end if;

  update public.word_tower_estates
    set raid_charges = case when p_revenge_raid_id is null then raid_charges - 1 else raid_charges end,
        updated_at = now()
    where player_id = p_attacker;
  update public.profiles
    set total_coins = coalesce(total_coins, 0) + v_gain,
        lifetime_coins_earned = coalesce(lifetime_coins_earned, 0) + v_gain,
        updated_at = now()
    where id = p_attacker;
