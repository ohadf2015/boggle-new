...
export async function findDailyChallengeRivals(
  ...
  for (const r of (puzzleRes.data ?? []) as Array<{ player_id: string | null }>) {
    if (r.player_id) completers.add(r.player_id);
  }
  ...
  const lbRes = await supabase
    .from('leaderboard')
    .select('player_id, username, avatar_image, total_score')
    .in('player_id', lbIds);
  ...
  const rivalPool: Array<{ id: string; username: string; avatar: string; score: number }> = [];
  ...
  for (const userId of recipientIds) {
    ...
    let best: RivalCandidate | null = null;
