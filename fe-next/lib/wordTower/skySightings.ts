 ...
 *
 * High up, the backdrop very occasionally produces a drifting "sighting" — a
 * cosmic whale gliding through the nebula, a satellite glint, a shooting star.
...
export type SightingKind = 'whale' | 'satellite' | 'shootingStar';

/** Transparent PNG / asset path per sighting (whale art is intentionally
 *  majestic-cosmic, not kawaii). Star + satellite are drawn in CSS by the
 *  component, so only the whale needs an asset. */
 ...
 * cheaper satellite / shooting-star glints.
 */
export function pickSighting(roll01: number, viewAltM: number): SightingKind | null {
  ...
  if (viewAltM >= SIGHTING_WHALE_MIN_ALT_M && sub < 0.4) return 'whale';
  return sub < 0.7 ? 'shootingStar' : 'satellite';
}
