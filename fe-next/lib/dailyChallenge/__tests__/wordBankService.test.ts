...
describe('importWikipediaWordsToBank', () => {
  ...
  describe('Length filtering for English', () => {
    ...
    it('should filter out words longer than 8 characters', async () => {
      const words = ['test', 'hello', 'wonderful', 'extraordinary']; // 4, 5, 9, 13 chars

      await importWikipediaWordsToBank(mockSupabaseClient, language, words);

      // Should only upsert words with 4-8 characters (test, hello = 2 calls)
      expect(mockUpsert).toHaveBeenCalledTimes(2);

      const upsertedWords = mockUpsert.mock.calls.map(call => call[0].word);
      expect(upsertedWords).toEqual(['TEST', 'HELLO']);
    });

    it('should pass all valid words (4-8 characters)', async () => {
      const words = ['test', 'hello', 'world', 'computer']; // 4, 5, 5, 8 chars
      ...
      const upsertedWords = mockUpsert.mock.calls.map(call => call[0].word);
      expect(upsertedWords).toEqual(['TEST', 'HELLO', 'WORLD', 'COMPUTER']);
    });
  });
  ...
  describe('Length filtering for Hebrew', () => {
    ...
    it('should filter words by Hebrew length requirements (4-8 chars)', async () => {
      const words = ['אני', 'אמא', 'אבא', 'מילה', 'תפוח']; // 3, 3, 3, 4, 4 chars

      await importWikipediaWordsToBank(mockSupabaseClient, language, words);

      // Should only upsert 4+ char words (מילה, תפוח = 2 calls)
      expect(mockUpsert).toHaveBeenCalledTimes(2);

      const upsertedWords = mockUpsert.mock.calls.map(call => call[0].word);
      expect(upsertedWords).toEqual(['מילה', 'תפוח']);
    });
  });
  ...
  describe('Return value', () => {
    it('should return correct counts', async () => {
      const words = ['test', 'hello', 'world', 'computer'];
  ...
  describe('Language-specific length ranges', () => {
    it('should respect WORD_LENGTH_RANGE configuration', () => {
      ...
      expect(WORD_LENGTH_RANGE.en).toEqual({ min: 4, max: 8 });
      expect(WORD_LENGTH_RANGE.he).toEqual({ min: 4, max: 8 });
      expect(WORD_LENGTH_RANGE.sv).toEqual({ min: 4, max: 8 });
      expect(WORD_LENGTH_RANGE.ja).toEqual({ min: 2, max: 4 });
      expect(WORD_LENGTH_RANGE.es).toEqual({ min: 4, max: 8 });
    });
