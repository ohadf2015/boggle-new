 ...
 *
 * Lives in the (home) route group so it ONLY covers the landing page. Sibling
 * routes (practice, blast, crossword, daily, SEO pages, …) fall through to the
 * generic mascot loader at [locale]/loading.tsx instead of this rich skeleton.
 *
 * Server Component: streams as HTML with zero JS, painted before hydration.
 * Mirrors LandingView's section order (season strip → hero+leaderboard →
 * social proof → cubes bento) so the skeleton→content swap reflows nothing.
 */
  ...
  return (
    <div className="flex-1 flex flex-col bg-gray-100 dark:bg-neo-navy relative page-content-safe h-full">
      ...
      <main className="w-full max-w-7xl mx-auto overflow-x-hidden relative z-20 flex-1 flex flex-col px-2 sm:px-3 lg:px-6 xl:px-8 py-3 sm:py-5 lg:py-8 gap-6 sm:gap-8">
        ...
        {/* Season strip — LandingSeasonHero sits ABOVE the hero in LandingView (:191).
