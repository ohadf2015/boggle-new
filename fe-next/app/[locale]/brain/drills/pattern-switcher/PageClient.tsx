 ...
 */
export default function PatternSwitcherPageClient() {
  ...
  const drillLevel = useDrillLevel('pattern-switcher');
  ...
  const [sessionId] = useState(() => `drill_pattern-switcher_${crypto.randomUUID()}`);
  ...
  React.useEffect(() => {
    ...
    trackDrillStart({ drillType: 'pattern-switcher', level: drillLevel });
    ...
    return (
      <div className={cn(
        ...
        <div
          ...
          className="w-12 h-12 border-4 border-t-transparent rounded-full border-neo-cyan motion-safe:animate-spin"
        />
      </div>
    );
  }
  ...
  return (
    <div dir={dir} className={cn(
      ...
      <DrillResearchIntro drillType="pattern-switcher" />

      {/* Drill Content */}
      <div className="flex-1">
        <FeatureErrorBoundary featureName="drill-pattern-switcher">
          <PatternSwitcher
            grid={grid}
