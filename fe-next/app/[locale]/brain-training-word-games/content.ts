...
const en: BrainLandingCopy = {
  ...
  drills: [
    ...
    { name: 'Memory Hunt', domain: 'Working Memory', tagline: 'Hold it in your head, then recall', blurb: 'A few target words light up on the grid for a few seconds. The highlights vanish, and you find those same words from memory — miss one and you lose a life, across up to 5 rounds with new words each time. Memory Hunt is a hold-and-find word recall task — the kind of short-term hold-and-use challenge working-memory research is built around.', research: 'Working-memory span tasks are among the most-studied paradigms in cognitive-training literature (e.g., NIH-indexed PMC5930973), though how far any gains transfer beyond the task itself is still debated.' },
  ...
  itemListDescriptions: [
    ...
    'Study a few highlighted words on the grid, then find them again from memory. Inspired by working-memory span tasks.',
...
const he: BrainLandingCopy = {
  ...
  drills: [
    ...
    { name: 'Memory Hunt', domain: 'זיכרון עבודה', tagline: 'תזכרו, ואז תיזכרו', blurb: 'כמה מילות יעד נדלקות על הרשת לכמה שניות. ההדגשה נעלמת, ואתם צריכים למצוא את אותן מילים מהזיכרון — טעות עולה לכם חיים, על פני עד 5 סבבים עם מילים חדשות בכל פעם. Memory Hunt הוא תרגיל של זכירת מילים ואיתורן מחדש — סוג האתגר שמחקר זיכרון העבודה בנוי סביבו.', research: 'משימות טווח זיכרון עבודה הן מהנחקרות ביותר בספרות המחקר (למשל מחקר ב-NIH, PMC5930973), אך עד כמה רווחים כאלה עוברים מעבר למשימה עצמה עדיין שנוי במחלוקת.' },
  ...
  faqs: [
    { q: 'האם משחקי אימון מוח באמת מבוססי מחקר?', a: 'התרגילים שלנו בנויים בהשראת משימות שחוקרים משתמשים בהן לבחון מיומנויות ספציפיות — למשל, שליפת מילים מהירה דומה למבחני שטף מילולי, וזכירת מילים ואיתורן מחדש דומה למשימות טווח זיכרון עבודה קלאסיות. זה שונה מלהגיד ש-LexiClash הוכיחה שהיא משפרת זיכרון, IQ או קוגניציה כללית — לאף אפליקציית אימון מוח צרכנית אין הוכחה מוצקה לכך, ואנחנו לא טוענים את זה. מה שאנחנו כן מודדים בכנות זה הביצועים שלכם במשימות הספציפיות האלה, דרך בדיקת המוח היומית.' },
    ...
    { q: 'אילו תחומים קוגניטיביים מטופלים על ידי 4 התרגילים?', a: 'Lightning Round הוא תרגיל מהירות מציאת מילים. Memory Hunt הוא תרגיל של זכירת מילים ואיתורן מחדש. Combo Master הוא תרגיל ריכוז ממושך. Rare Gems הוא תרגיל עומק אוצר מילים. כל אחד בנוי בהשראת סוג משימה שחוקרים משתמשים בה כדי לבחון את המיומנות הזו, ובדיקת המוח עוקבת אחרי המגמה שלכם בכל אחד — אין ציון משולב מ-0 עד 100.' },
  ...
  itemListDescriptions: [
    ...
    'שננו כמה מילים שמודגשות לרגע על הרשת, ואז מצאו אותן שוב מהזיכרון. בהשראת משימות טווח זיכרון עבודה.',
...
const sv: BrainLandingCopy = {
  ...
  drills: [
    ...
    { name: 'Memory Hunt', domain: 'Arbetsminne', tagline: 'Behåll i minnet, sedan minns', blurb: 'Några målord lyser upp på rutnätet i några sekunder. Markeringarna försvinner, och du hittar samma ord igen ur minnet — ett fel kostar ett liv, över upp till 5 omgångar med nya ord varje gång. Memory Hunt är en håll-och-hitta-uppgift för ordigenkallning — den typ av kortvarigt håll-och-använd-utmaning som arbetsminnesforskning bygger på.', research: 'Uppgifter om arbetsminnesspann hör till de mest studerade paradigmen i forskningslitteraturen (t.ex. NIH-indexerad PMC5930973), men hur långt eventuella vinster sträcker sig bortom själva uppgiften diskuteras fortfarande.' },
  ...
  faqs: [
    { q: 'Är hjärnträningsspel verkligen forskningsbaserade?', a: 'Våra övningar är inspirerade av uppgifter forskare använder för att studera specifika förmågor — snabb ordhämtning liknar till exempel test av verbal flyt, och att hålla kvar ord och hitta dem igen liknar klassiska arbetsminnesuppgifter. Det är något annat än att säga att LexiClash i sig har bevisats förbättra minne, IQ eller allmän kognition — ingen konsumentapp för hjärnträning har robusta bevis för det, och vi påstår inte det. Det vi mäter ärligt är dina resultat på just dessa uppgifter, via daglig Hjärnkoll.' },
    ...
    { q: 'Vilka kognitiva områden tränar de 4 hjärnövningarna?', a: 'Lightning Round är en uppgift för att hitta ord snabbt. Memory Hunt är en håll-och-hitta-uppgift för ord. Combo Master är en uppgift för uthållig uppmärksamhet. Rare Gems är en ordförrådsdjupsövning. Var och en är inspirerad av en typ av uppgift forskare använder för att studera just den förmågan, och Hjärnkoll följer din egen trend på var och en — det finns ingen sammanslagen poäng 0-100.' },
  ...
  itemListDescriptions: [
    ...
    'Studera några markerade ord på rutnätet, hitta dem sedan igen ur minnet. Inspirerad av uppgifter om arbetsminnesspann.',
...
const ja: BrainLandingCopy = {
  ...
  drills: [
    ...
    { name: 'Memory Hunt', domain: 'ワーキングメモリ', tagline: '頭に保持し、後で思い出す', blurb: '数個の対象ワードがグリッド上で数秒間光る。ハイライトが消えたら、同じ単語を記憶を頼りに見つける — 間違えるとライフが1つ減り、最大5ラウンドまで、毎回新しい単語で続く。Memory Huntは保持して見つける単語記憶課題 — ワーキングメモリ研究の土台となっている、短期保持と操作の課題です。', research: 'ワーキングメモリのスパン課題は文献の中で最も研究されているパラダイムの一つです (NIH掲載のPMC5930973など)。ただし課題そのものを超えてどこまで効果が及ぶかは今も議論が続いています。' },
  ...
  faqHeading: 'よくある質問',
  faqs: [
    { q: '脳トレ単語ゲームは本当に研究に裏付けられていますか?', a: '私たちのドリルは、研究者が特定のスキルを調べるために使う課題からヒントを得ています — 例えば素早い単語検索は言語流暢性テストに似ており、単語を保持してから見つけ出す課題は古典的なワーキングメモリのスパン課題に似ています。これは、LexiClash自体が記憶力、IQ、全般的な認知能力を向上させると証明された、というのとは違います。市販の脳トレアプリにそれを裏付ける確かな証拠はなく、私たちもそう主張しません。正直に測定しているのは、毎日のブレインチェックを通じたこれらの課題そのものでの成績です。' },
    ...
    { q: '4つの脳ドリルが対象とする認知分野は何ですか?', a: 'Lightning Roundは単語を素早く見つける課題です。Memory Huntは単語を保持して見つけ出す記憶課題です。Combo Masterは持続的注意の課題です。Rare Gemsは語彙の深さの課題です。それぞれ研究者がそのスキルを調べるために使う課題の種類からヒントを得ており、ブレインチェックがそれぞれの傾向を追跡します — 統合された0〜100のスコアはありません。' },
  ...
  itemListDescriptions: [
    ...
    'グリッド上で光った単語をいくつか覚え、それから記憶を頼りに見つけ出す。ワーキングメモリのスパン課題からヒントを得ています。',
...
const es: BrainLandingCopy = {
  ...
  drills: [
    ...
    { name: 'Memory Hunt', domain: 'Memoria de trabajo', tagline: 'Mantenlo en la cabeza, luego recuérdalo', blurb: 'Unas pocas palabras objetivo se iluminan en la cuadrícula durante unos segundos. Los resaltados desaparecen y tienes que encontrar esas mismas palabras de memoria — fallar una te cuesta una vida, a lo largo de hasta 5 rondas con palabras nuevas cada vez. Memory Hunt es una tarea de retener y encontrar palabras de memoria — el tipo de desafío de retención y uso a corto plazo en el que se basa la investigación de la memoria de trabajo.', research: 'Las tareas de amplitud de memoria de trabajo están entre los paradigmas más estudiados en la literatura (por ejemplo, la investigación indexada en NIH PMC5930973), aunque hasta qué punto las ganancias se transfieren más allá de la propia tarea sigue siendo debatido.' },
  ...
  faqs: [
    { q: '¿Los juegos de entrenamiento cerebral con palabras están realmente respaldados por investigación?', a: 'Nuestros ejercicios están inspirados en tareas que los investigadores usan para estudiar habilidades específicas — por ejemplo, la recuperación rápida de palabras se parece a las pruebas de fluidez verbal, y retener palabras para luego encontrarlas se parece a las clásicas tareas de amplitud de memoria de trabajo. Eso es distinto de decir que LexiClash en sí ha demostrado mejorar la memoria, el IQ o la cognición general — ninguna app de entrenamiento cerebral para consumidores tiene evidencia sólida de eso, y nosotros tampoco lo afirmamos. Lo que sí medimos con honestidad es tu rendimiento en estas tareas específicas, mediante el Chequeo mental diario.' },
    ...
    { q: '¿Qué áreas cognitivas abordan los 4 ejercicios cerebrales?', a: 'Lightning Round es una tarea de velocidad para encontrar palabras. Memory Hunt es una tarea de retener y encontrar palabras de memoria. Combo Master es una tarea de atención sostenida. Rare Gems es una tarea de profundidad de vocabulario. Cada uno está inspirado en un tipo de tarea que los investigadores usan para estudiar esa habilidad, y el Chequeo mental sigue tu propia tendencia en cada una — no hay un puntaje combinado de 0 a 100.' },
  ...
  itemListDescriptions: [
    ...
    'Estudia unas pocas palabras resaltadas en la cuadrícula, luego encuéntralas de nuevo de memoria. Inspirado en tareas de amplitud de memoria de trabajo.',
...
const ru: BrainLandingCopy = {
  ...
  itemListDescriptions: [
    ...
    'Изучи несколько подсвеченных слов на сетке, затем найди их снова по памяти. Вдохновлено заданиями на объём рабочей памяти.',
