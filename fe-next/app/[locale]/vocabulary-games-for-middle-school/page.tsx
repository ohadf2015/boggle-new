...
import { getMiddleSchoolContent } from './content';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const c = getMiddleSchoolContent(locale);
...
export default async function Page({ params }: PageProps) {
  ...
  const moreHrefs = [
    ...
    `/${locale}/education/esl-word-games`,
    `/${locale}/education`,
  ];
    ...
    <main className="min-h-screen bg-neo-navy text-neo-white">
      ...
      <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <section className="mb-12 flex flex-col gap-3 sm:flex-row sm:gap-4">
          ...
          <Link href={`/${locale}/education`} className="rounded-neo border-4 border-neo-pink bg-transparent px-6 py-3 text-center font-bold text-neo-pink shadow-hard transition-all hover:bg-neo-pink/10 sm:px-8 sm:py-4">
            {c.ctaMore}
          </Link>
        </section>
