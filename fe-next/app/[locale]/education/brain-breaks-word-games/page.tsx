...
import { getBrainBreaksContent } from './content';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildEducationLandingMetadata({ locale, path: PATH, content: getBrainBreaksContent(locale) });
}

export default async function Page({ params }: PageProps) {
  const { locale } = await params;
  return <EducationLandingTemplate locale={locale} path={PATH} content={getBrainBreaksContent(locale)} />;
}
