...
const content: Record<EducationLocale, LocaleContent> = {
  en: {
    ...
    faqs: [
      ...
      { q: 'Is LexiClash classroom-ready for school spelling bees?', a: 'Yes. Teachers can run whole-class spelling-bee warm-ups (a whole class join with a 4-digit code, no student accounts), 1v1 elimination brackets, or assign solo practice. The teacher dashboard shows per-student accuracy and which words tripped the most competitors — useful for targeting review sessions.' },
  ...
  he: {
    ...
    faqs: [
      ...
      { q: 'מתאים לכיתה שלמה?', a: 'כן. מורים יכולים להנהל תרגול לכל הכיתה (כל הכיתה עם קוד 4 ספרות, בלי חשבונות), תחרויות דו-קרב או תרגול בודד. לוח המורה מראה דיוק לתלמיד ואילו מילים טרידו את רוב המתחרים — כך אתם יודעים מה לתרגל בחזרה.' },
  ...
  ja: {
    ...
    faqs: [
      ...
      { q: 'LexiClashは学校のスペリング競争に対応していますか？', a: 'はい。教師は全クラスのスペリング暖房を実行できます（クラス全員が4桁のコードで参加、学生アカウントなし）、1v1排除戦、またはソロ練習を割り当てます。教師のダッシュボードは生徒ごとの精度と、ほとんどの競争者がつまずいた単語を示しています — レビューセッションのターゲット化に役立ちます。' },
  ...
  es: {
    ...
    faqs: [
      ...
      { q: '¿Está LexiClash lista para competencias de ortografía en escuelas?', a: 'Sí. Los maestros pueden ejecutar calentamientos de ortografía para toda la clase (toda la clase se unen con un código de 4 dígitos, sin cuentas de estudiante), torneos de eliminación 1v1 o asignar práctica solo. El panel del maestro muestra la precisión por estudiante y qué palabras tropezaron con la mayoría de los competidores — útil para dirigirse a sesiones de revisión.' },
