...
const SECTIONS: Record<EducationLocale, ClassGameSection> = {
  ...
  sv: {
    heading: '6 engelsklekar för lågstadiet',
    intro: 'Sex korta lekar för unga engelskinlärare, från en tyst bilduppvärmning till en delad bokstavsplan. De som inte kräver någon enhet säger det på sin rad. Resten körs i webbläsaren och bedöms mot engelska, inte en översatt lista.',
  ...
  ja: {
    heading: '小学校の英語クラス向け6ゲーム',
    intro: '絵を使った静かなウォームアップから共有の文字盤まで、英語を学び始めた子ども向けの短いゲーム6つ。端末が要らないものは各行に明記します。ほかはブラウザで、翻訳リストではなく英語の辞書で判定されます。',
  ...
  ru: {
    heading: '6 игр на английском для начальной школы',
    intro: 'Шесть коротких игр для младших, от тихой разминки с картинкой до общего буквенного поля. Те, что без устройства, говорят об этом в своей строке. Остальные идут в браузере и судятся по английскому, а не по переводному списку.',
...
export function getElementaryClassGames(locale: string): ClassGameSection {
