...
const nextConfig = {
  ...
  async headers() {
    return [
      ...
      {
        source: '/:path((?!(?:en|he|sv|ja|es|ru)/(?:embed/|education/classroom-addon|education/miss-gap-grade-passback|education/unplugged-grade-passback)).*)',
        headers: [
      ...
      // Unplugged reteach Live grade passback receipt (Classroom iframe).
      {
        source: '/:locale(en|he|sv|ja|es|ru)/education/unplugged-grade-passback',
        headers: [
          { key: 'Strict-Transport-Security', value: 'max-age=31536000; includeSubDomains; preload' },
          { key: 'X-Content-Type-Options', value: 'nosniff' },
          { key: 'Referrer-Policy', value: 'strict-origin-when-cross-origin' },
