...
export function useWeeklyChest(): WeeklyChestState {
  ...
  useEffect(() => {
    ...
    // Guest home mounted this hook and GET /api/daily/weekly-chest/status
    // returned 401 on every anonymous pageview. Wait for auth to resolve,
    // then skip the request for guests (UI already falls back to local streak
    // when cycleStart is empty).
    if (!authLoading) {
      if (!isAuthenticated) {
        setLoading(false)
      } else {
