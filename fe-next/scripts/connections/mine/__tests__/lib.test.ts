...
describe('lib.mjs - Deterministic Puzzle Miner', () => {
  ...
  describe('buildBridgeGraph', () => {
    it('should build a graph mapping bridge -> Set of (word1, word2) pairs', () => {
      // A bridge needs both suffix and prefix uses to appear in the graph
      const splits = new Set([
        { compound: 'football', left: 'foot', right: 'ball' },
        { compound: 'ballroom', left: 'ball', right: 'room' },
      ]);
      const graph = buildBridgeGraph(splits);

      // "ball" is used as a suffix (foot + ball) and prefix (ball + room)
      expect(graph.has('ball')).toBe(true);

      const ballPairs = graph.get('ball');
      expect(ballPairs.size).toBeGreaterThan(0);

      // Should generate (foot, room) pair
      const pair = [...ballPairs][0];
      expect(pair.word1).toBe('foot');
      expect(pair.word2).toBe('room');
    });

    it('should only include bridges that work both as suffix and prefix', () => {
      // "pie" only used as suffix (apple + pie), no prefix use
      const splits = new Set([
        { compound: 'applepie', left: 'apple', right: 'pie' },
      ]);
      const graph = buildBridgeGraph(splits);

      // "pie" should NOT be in the graph since it has no prefix usage
      expect(graph.has('pie')).toBe(false);
    });
  });
