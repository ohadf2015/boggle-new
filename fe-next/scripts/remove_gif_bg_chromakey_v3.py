#!/usr/bin/env python3
"""
GIF background removal using chromakey with BINARY transparency only.
No feathering or smoothing - GIF format requires fully transparent or fully opaque pixels.

Usage:
    python3 scripts/remove_gif_bg_chromakey_v3.py <input.gif> [output.gif]
    python3 scripts/remove_gif_bg_chromakey_v3.py --batch <directory>
"""
...
def remove_color_binary(
    frame: np.ndarray,
    target_color: Tuple[int, int, int] = (0, 0, 0),
    tolerance: int = 40
) -> Image.Image:
    """
    Remove a specific color from a frame with BINARY transparency only.
    No gradients, no semi-transparent pixels - prevents GIF trails.
    ...
    Returns:
        PIL Image with binary transparent background
    """
    ...
    # Process each pixel with BINARY transparency only
    for y in range(height):
        for x in range(width):
            r, g, b, a = pixels[x, y]

            # Simple color distance (Manhattan distance)
            color_distance = (
                abs(r - target_r) +
                abs(g - target_g) +
                abs(b - target_b)
            ...
            # Binary decision: fully transparent or fully opaque
            if color_distance <= tolerance:
                # Match - make fully transparent
                pixels[x, y] = (0, 0, 0, 0)
            else:
                # No match - keep fully opaque
...
def process_frame(
    ...
    return remove_color_binary(frame, target_color, tolerance)
...
def process_gif_file(
    ...
    """
    Process a single GIF file to remove background color with binary transparency.
    ...
    try:
        ...
        print(f"✨ Mode: BINARY transparency (no trails)")
...
def main():
    parser = argparse.ArgumentParser(
        description='GIF background removal with BINARY transparency (no trails).',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Remove black background
  python3 scripts/remove_gif_bg_chromakey_v3.py public/mascot/main.gif

  # Custom tolerance
  python3 scripts/remove_gif_bg_chromakey_v3.py input.gif output.gif --tolerance 50

  # Batch process all GIFs
  python3 scripts/remove_gif_bg_chromakey_v3.py --batch public/mascot/

  # Remove white background
  python3 scripts/remove_gif_bg_chromakey_v3.py input.gif --color 255 255 255
        """
    )
