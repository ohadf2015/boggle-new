...
interface WordDetailWithTiming extends WordDetail {
 ...
 * @param scoresArray - Array of player score objects with allWords, validWordCount, etc.
 * @param locale - The game language for localized titles
 * @param gameDuration - Game duration in seconds
 * @returns Map of username -> { titleKey, title (localized object) }
 */
export function calculatePlayerTitles(
  scoresArray: PlayerScoreInput[],
  locale: string = 'he',
  gameDuration: number = 180
): Record<string, AssignedTitle> {
  ...
  const assignedTitles: Record<string, AssignedTitle> = {};
