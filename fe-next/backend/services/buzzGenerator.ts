...
function filterTrends(
  ...
  // Maximum sports trends to allow in the filtered results
  const MAX_SPORTS_IN_FILTER = 1;
  ...
  // This ensures topic diversity by preventing sports domination
  let sportsCount = 0;
  ...
  return diverseFiltered.slice(0, 10); // Keep top 10 filtered trends
}
...
// Sports keywords for detecting sport riddles (used in validation) - English only, legacy
 ...
 * Used to identify sports-related trends that should be deprioritized for variety
 */
 ...
 * Uses multi-language sports keywords to detect sports trends accurately
 *
 * @param query - The trend query to check
 * @param language - The language code (en, he, sv, ja, es)
 * @returns true if the query contains sports keywords
 */
function isSportsRelatedTrend(query: string, language: string): boolean {
  const lowercaseQuery = query.toLowerCase();

  // Get sports keywords for the specific language
  const languageKeywords = SPORTS_KEYWORDS_BY_LANGUAGE[language] || [];

  // Also check English keywords (many sports terms are used in English globally)
  const englishKeywords = SPORTS_KEYWORDS_BY_LANGUAGE.en || [];

  // Combine both language-specific and English keywords
  const allKeywords = [...new Set([...languageKeywords, ...englishKeywords])];
