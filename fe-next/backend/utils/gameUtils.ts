...
// Russian (Cyrillic) — board uses UPPERCASE to match the en/sv/es convention;
// the dictionary is lowercase and validation lowercases the guess (Russian has
// case, unlike Hebrew/Japanese). Hard sign Ъ is excluded from the board: it
// never starts a word and is ~0.04% frequency — a pure dead tile.
...
// boards. Same approach as spanishLetterPool. Verified end-to-end in Phase 5
// (board renders Cyrillic + a real Russian word validates).
export const russianLetterPool: string[] = (() => {
  const weights: Record<string, number> = {
    ...
    // Ё is intentionally absent: it folds to Е (ё→е) at validate time so the
    // board never shows a tile that can't form a stored word.
  };
  const pool: string[] = [];
