...
import { emitToUser, joinUserToRoom } from './userSockets';
...
export function registerLifecycleHandlers(
  ...
  socket.on('duel:create', async (data: unknown) => {
    ...
    try {
      ...
      // Notify EVERY socket the opponent has open — the lobby and the
      // notification banner each hold their own (see ./userSockets).
  ...
  socket.on('duel:accept', async (data: unknown) => {
    ...
    try {
      ...
      // ALL of the challenger's sockets join — `duel:started` is emitted to
      // this room once, and the socket their lobby listens on must be in it.
      joinUserToRoom(namespace, duel.challenger_id, duelRoom);
  ...
  socket.on('duel:decline', async (data: unknown) => {
    ...
    try {
      ...
      emitToUser(namespace, duel.challenger_id, 'duel:declined', {
        duelId: payload.duelId,
      });
  ...
  socket.on('duel:cancel', async (data: unknown) => {
    ...
    try {
      ...
      emitToUser(namespace, duel.opponent_id, 'duel:cancelled', {
        duelId: payload.duelId,
      });
