...
describe('Multiplayer Lobby Mobile Bottom Tabs', () => {
  describe('HostPreGameView', () => {
    ...
    it('should have start button section visible on mobile and sticky', () => {
      ...
      // Find the start button container (the section above the tab bar)
      const startButton = screen.getByRole('button', { name: /startGame/i });
      const startButtonContainer = startButton.parentElement;

      // Check container has classes for sticky positioning
      expect(startButtonContainer).toHaveClass('flex-shrink-0');

      // The start button should be in a section that's visible on mobile
      // but the container should have lg:hidden to hide on desktop
      expect(startButtonContainer?.className).toContain('lg:hidden');
    });
