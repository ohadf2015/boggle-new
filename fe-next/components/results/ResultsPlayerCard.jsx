...
// Neo-Brutalist color mapping based on points
// Using solid Neo colors for visual hierarchy
const POINT_COLORS = {
...
const ResultsPlayerCard = ({ player, index, allPlayerWords, currentUsername, isWinner }) => {
  ...
  const { duplicateWords, invalidWords, wordsByPoints, sortedPointGroups, totalComboBonus } = useMemo(() => {
    ...
    if (totalComboBonus > 0) {
      console.log(`[RESULTS] ${player.username} combo bonus: ${totalComboBonus} from ${validWords.filter(w => w.comboBonus > 0).length} words with bonuses`);
    }
