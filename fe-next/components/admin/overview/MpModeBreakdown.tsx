...
import { Gamepad2, Bomb, Target, RotateCw } from 'lucide-react';
import type { MpModeBreakdownStat } from '@/lib/admin/fetchMpModeBreakdown';

const MODE_CONFIG: Record<string, { icon: React.ElementType; color: string; barColor: string }> = {
  classic: { icon: Gamepad2, color: 'text-neo-pink', barColor: 'bg-neo-pink' },
  blast: { icon: Bomb, color: 'text-neo-cyan', barColor: 'bg-neo-cyan' },
  'word-hunt': { icon: Target, color: 'text-neo-lime', barColor: 'bg-neo-lime' },
  'wheel-rush': { icon: RotateCw, color: 'text-neo-purple', barColor: 'bg-neo-purple' },
};
...
export function MpModeBreakdown() {
  const { t } = useLanguage();
  const [stats, setStats] = useState<MpModeBreakdownStat[] | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`/api/game-mode-stats?days=${days}`)
      .then(res => res.json())
      .then(data => setStats(data.mpBreakdown ?? []))
      .catch(() => setStats(null))
      ...
      {loading ? (
        <div className="space-y-3">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={`skel-${i}`} className="h-8 bg-neo-navy-elevated/50 rounded-neo animate-pulse" role="presentation" />
          ))}
        </div>
      ) : stats && stats.length > 0 ? (
        <div className="space-y-2.5">
          {stats.map((stat, index) => {
            const config = MODE_CONFIG[stat.mode] || MODE_CONFIG.classic;
            ...
            const labelKey = MODE_LABEL_KEYS[stat.mode] || stat.mode;
