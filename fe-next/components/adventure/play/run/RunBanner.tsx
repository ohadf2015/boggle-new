    ...
    <section className="mx-3 mt-3 rounded-2xl border-[3px] border-black bg-[#1b2a5c] p-3 text-neo-cream shadow-[4px_4px_0_#000]" data-testid="run-banner">
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          ...
          <div className="font-neo-display text-lg font-bold leading-tight">{t('adventurePlay.loot.runStep', { step: run.step, total: LEVELS_PER_WORLD })}</div>
        </div>
      ...
      <ol className="mt-2 flex gap-1" aria-hidden>
        {Array.from({ length: LEVELS_PER_WORLD }, (_, i) => i + 1).map((n) => (
