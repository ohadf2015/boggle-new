...
describe('useBlastObjectives', () => {
  ...
  describe('Goal Gallery — cascade_chain objective', () => {
    const cascadeObj = { type: 'cascade_chain' as const, target: 3 };

    it('reads progress from peakCascadeDepth', () => {
      const gameState = makeGameState({ peakCascadeDepth: 2, cascadeChainLevel: 0 });
      const { result } = renderHook(() =>
        useBlastObjectives({
          gameState,
          tileTypeClears: {} as Record<BlastTileType, number>,
          objectives: [cascadeObj],
          wordsFound: [],
        }),
      );
    ...
    it('marks complete when peak ≥ target even after later non-cascading moves', () => {
      // Player triggered cascade depth 3 once; later moves no longer cascade
      const gameState = makeGameState({ peakCascadeDepth: 3, cascadeChainLevel: 0 });
      const { result } = renderHook(() =>
        useBlastObjectives({
          gameState,
          tileTypeClears: {} as Record<BlastTileType, number>,
    ...
    it('returns 0 when no chain ever attempted', () => {
      const { result } = renderHook(() =>
        useBlastObjectives({
          gameState: makeGameState(),
          tileTypeClears: {} as Record<BlastTileType, number>,
  ...
  describe('Goal Gallery — path_route objective', () => {
    const routeObj = {
      type: 'path_route' as const,
      target: 1,
      startCell: { row: 0, col: 0 },
      endCell: { row: 5, col: 5 },
    };

    it('current=0 isComplete=false when routeCompleted unset', () => {
      const { result } = renderHook(() =>
        useBlastObjectives({
          gameState: makeGameState(),
          tileTypeClears: {} as Record<BlastTileType, number>,
    ...
    it('current=1 isComplete=true when routeCompleted', () => {
      const { result } = renderHook(() =>
        useBlastObjectives({
          gameState: makeGameState({ routeCompleted: true }),
          tileTypeClears: {} as Record<BlastTileType, number>,
          objectives: [routeObj],
          wordsFound: [],
        }),
      );
      expect(result.current.objectiveProgress[0].current).toBe(1);
      expect(result.current.objectiveProgress[0].isComplete).toBe(true);
    });
  });

  describe('Goal Gallery — tile_sniper objective', () => {
    const sniperObj = {
      type: 'tile_sniper' as const,
      target: 1,
      targetCell: { row: 2, col: 3 },
    };

    it('returns 0 when sniperHit unset', () => {
      const { result } = renderHook(() =>
        useBlastObjectives({
          gameState: makeGameState(),
          tileTypeClears: {} as Record<BlastTileType, number>,
          objectives: [sniperObj],
          wordsFound: [],
        }),
      );
      expect(result.current.objectiveProgress[0].isComplete).toBe(false);
    ...
    it('marks complete when sniperHit flag set', () => {
      const { result } = renderHook(() =>
        useBlastObjectives({
          gameState: makeGameState({ sniperHit: true }),
          tileTypeClears: {} as Record<BlastTileType, number>,
          objectives: [sniperObj],
          wordsFound: [],
        }),
  ...
  describe('Goal Gallery — long_word_lockup objective', () => {
    const lockupObj = {
      type: 'long_word_lockup' as const,
      target: 1,
      minWordLength: 8,
    };

    it('incomplete when peakWordLength below threshold', () => {
      const { result } = renderHook(() =>
        useBlastObjectives({
          gameState: makeGameState({ peakWordLength: 7 }),
          tileTypeClears: {} as Record<BlastTileType, number>,
          objectives: [lockupObj],
          wordsFound: [],
        }),
      );
      expect(result.current.objectiveProgress[0].current).toBe(0);
    ...
    it('complete when peakWordLength meets threshold', () => {
      const { result } = renderHook(() =>
        useBlastObjectives({
          gameState: makeGameState({ peakWordLength: 9 }),
          tileTypeClears: {} as Record<BlastTileType, number>,
          objectives: [lockupObj],
          wordsFound: [],
        }),
    ...
    it('defaults minWordLength to 8 when omitted', () => {
      const objNoMin = { type: 'long_word_lockup' as const, target: 1 };
      const { result } = renderHook(() =>
        useBlastObjectives({
          gameState: makeGameState({ peakWordLength: 8 }),
          tileTypeClears: {} as Record<BlastTileType, number>,
          objectives: [objNoMin],
          wordsFound: [],
        }),
