/**
 * BlastGameLayoutPhaser — portrait layout for Blast Mode with Phaser canvas grid.
 *
 * Identical UI chrome to BlastGameLayout (header, combo, score, word forming,
 * dialogs, found words) but replaces the DOM-based BlastGrid with a Phaser
 * canvas (BlastPhaserCanvas) loaded via dynamic import (ssr: false).
 *
 * Grid interaction props (onWordSubmit, onPathSubmit, etc.) are handled
 * through GameBridge rather than as React props.
 */
...
const BlastPhaserCanvas = dynamic(() => import('@/components/phaser/BlastPhaserCanvas'), {
  ssr: false,
});

// ─── Props ───────────────────────────────────────────────────────────────────

interface BlastGameLayoutPhaserProps {
  // Game state
  ...
  comboDanger: boolean;
  // Word forming (from bridge)
  formedWord: string;
  currentFeedback: WordFeedback | null;
  // Cascade (React UI announcements only)
...
export function BlastGameLayoutPhaser({
  gameState,
  noWordsRemaining,
  comboLevel,
  comboTimeRemaining,
  ...
  return (
    <div className="relative flex-1 flex flex-col overflow-hidden h-full bg-neo-navy">
      ...
      {/* ── Phaser Canvas Grid Area (replaces BlastGrid) ──────────────────── */}
      <div className={cn(
        'flex-1 flex flex-col items-center justify-start px-4 pt-2 relative z-30 min-h-0 transition-shadow duration-500',
        comboGlow,
      )}>
        ...
        {/* Phaser canvas fills the grid area */}
