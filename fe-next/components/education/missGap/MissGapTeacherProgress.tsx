...
export function MissGapTeacherProgress({ data, failed }: MissGapTeacherProgressProps) {
  ...
  return (
    <section
      ...
      className="p-4 rounded-neo border-[3px] border-neo-cream bg-neo-navy-light shadow-hard"
    >
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        <p className="rounded-neo border-[3px] border-neo-cream bg-neo-navy px-3 py-2">
      ...
      {failed ? (
        <p className="mt-3 rounded-neo border-[3px] border-neo-black bg-neo-pink px-3 py-2 font-bold text-sm text-neo-black">
          {t('education.homework.teacherLoadFailed')}
        </p>
      ) : runs.length === 0 ? (
        // Counts without names means the caller is not signed in — the route
        // only hands the roster to an authenticated teacher. Saying "nobody has
        // played" next to "2 finished" would be a flat contradiction.
        <p
          data-testid="miss-gap-teacher-empty"
          className="mt-3 font-neo-body text-sm text-neo-cream"
        >
          {(data?.players ?? 0) > 0
            ? t('education.homework.teacherNamesSignedOut')
        ...
        <ul className="mt-3 space-y-1.5 max-h-56 overflow-y-auto pe-1">
          {runs.map((run, i) => (
            <li
              ...
              className="flex items-center gap-2 px-3 py-2 rounded-neo border-[3px] border-neo-cream bg-neo-navy"
            >
              ...
              <span className="font-neo-body text-xs text-neo-cream tabular-nums">
              ...
              <span
                className={cn(
                  'px-1.5 py-0.5 rounded border-[2px] border-neo-black text-[10px] font-bold uppercase',
