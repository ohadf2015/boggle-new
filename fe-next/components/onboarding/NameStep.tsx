 ...
 * Users enter their display name (2-20 characters, alphanumeric + spaces)
 */
const NameStep: React.FC<NameStepProps> = ({ name, onNameChange }) => {
  ...
  const minLength = 2;
  const maxLength = 20;
  const isValidFormat = /^[a-zA-Z0-9\s]+$/.test(name) || name === '';
  ...
  return (
    <div className="flex flex-col items-center space-y-6">
      ...
      >
        <div
          className={cn(
            'bg-neo-cream border-3 border-neo-black rounded-neo p-4 sm:p-6 shadow-hard-md',
            'transition-all',
        ...
        >
          <div className="flex items-center gap-3 mb-4">
            <div className="w-12 h-12 bg-neo-yellow border-2 border-neo-black rounded-full flex items-center justify-center shadow-hard-sm">
              <FaUser className="text-neo-black text-xl" />
            </div>
            <div className="flex-1">
              <input
                ...
                maxLength={maxLength}
                className={cn(
                  'w-full px-4 py-3 bg-white border-3 border-neo-black rounded-neo',
                  'font-bold text-lg text-neo-black placeholder:text-neo-black/40',
            ...
            {name.length > 0 && (
              <motion.div
                ...
                animate={{ scale: 1 }}
                className={cn(
                  'w-10 h-10 rounded-full border-2 flex items-center justify-center shadow-hard-sm',
                  isValid
      ...
      <motion.div
        ...
        className="bg-neo-pink border-3 border-neo-black rounded-neo p-4 shadow-hard-md max-w-md"
      >
        <p className="text-center text-sm text-neo-black">
