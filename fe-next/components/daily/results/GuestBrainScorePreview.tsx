'use client';

/**
 * GuestBrainScorePreview Component
 *
 * Shows anonymous users a locked preview of what their brain score WOULD be.
 * Creates FOMO and incentivizes signup to track cognitive progress.
 */

import React from 'react';
import { m } from 'framer-motion';
import { Brain, Lock, TrendingUp, Zap, Target, Lightbulb, BookOpen } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Loader } from '@/components/ui/Loader';
import { signInWithGoogle, signInWithDiscord } from '@/lib/supabase';

export interface GuestBrainScorePreviewProps {
  /** Estimated brain score points from this game (can be a rough estimate) */
  estimatedScore?: number;
  /** Translation function */
  t: (key: string, params?: Record<string, string | number>) => string;
  className?: string;
}

// Brand icons
const GoogleIcon = ({ className }: { className?: string }) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor" width="1em" height="1em">
    <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
    <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
    <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
    <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
  </svg>
);

const DiscordIcon = ({ className }: { className?: string }) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor" width="1em" height="1em">
    <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z"/>
  </svg>
);

// Cognitive domain icons
const DOMAIN_ICONS = [
  { icon: Zap, label: 'Processing', color: 'text-yellow-500' },
  { icon: Target, label: 'Attention', color: 'text-red-500' },
  { icon: Lightbulb, label: 'Flexibility', color: 'text-green-500' },
  { icon: Brain, label: 'Memory', color: 'text-purple-500' },
  { icon: BookOpen, label: 'Vocabulary', color: 'text-blue-500' },
];

export const GuestBrainScorePreview: React.FC<GuestBrainScorePreviewProps> = ({
  estimatedScore = 42,
  t,
  className = '',
}) => {
  const [isLoading, setIsLoading] = React.useState<string | null>(null);

  const handleSignIn = async (provider: 'google' | 'discord') => {
    setIsLoading(provider);
    try {
      if (provider === 'google') {
        await signInWithGoogle();
      } else {
        await signInWithDiscord();
      }
    } catch {
      setIsLoading(null);
    }
  };

  return (
    <m.div
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      className={`relative bg-neo-purple dark:from-purple-900/20 dark:to-indigo-900/20 rounded-neo border-2 border-purple-200 dark:border-purple-700 overflow-hidden ${className}`}
    >
      {/* Blurred preview content */}
      <div className="relative p-5">
        {/* Blur overlay */}
        <div className="absolute inset-0 bg-neo-cream/30 dark:bg-neo-navy/80 z-10 flex flex-col items-center justify-center p-4">
          <div className="bg-white dark:bg-neo-navy rounded-full p-3 shadow-lg mb-3">
            <Lock className="w-6 h-6 text-purple-600 dark:text-purple-400" />
          </div>
          <h3 className="text-lg font-black text-purple-800 dark:text-purple-200 text-center mb-1">
            {t('daily.brainScoreLocked')}
          </h3>
          <p className="text-sm text-purple-600 dark:text-purple-300 text-center mb-4">
            {t('daily.historyTeaser')}
          </p>

          {/* Sign up buttons */}
          <div className="w-full max-w-xs space-y-2">
            <Button
              onClick={() => handleSignIn('google')}
              disabled={!!isLoading}
              size="sm"
              className="w-full bg-white hover:bg-gray-50 text-gray-700 border-2 border-gray-200 rounded-neo shadow-hard-sm"
            >
              {isLoading === 'google' ? (
                <Loader size="sm" />
              ) : (
                <GoogleIcon className="w-4 h-4 me-2" />
              )}
              {t('auth.continueWithGoogle')}
            </Button>
            <Button
              onClick={() => handleSignIn('discord')}
              disabled={!!isLoading}
              size="sm"
              className="w-full bg-brand-discord hover:bg-brand-discord-hover text-white border-2 border-brand-discord-hover rounded-neo shadow-hard-sm"
            >
              {isLoading === 'discord' ? (
                <Loader size="sm" />
              ) : (
                <DiscordIcon className="w-4 h-4 me-2" />
              )}
              {t('auth.continueWithDiscord')}
            </Button>
          </div>
        </div>

        {/* Background preview (blurred) */}
        <div className="pointer-events-none select-none">
          {/* Header */}
          <div className="flex items-center gap-3 mb-4">
            <div className="p-2 bg-neo-purple/20 dark:bg-neo-purple/30 rounded-lg">
              <Brain className="w-6 h-6 text-purple-600 dark:text-purple-400" />
            </div>
            <div>
              <div className="font-black text-purple-800 dark:text-purple-200">Brain Score</div>
              <div className="flex items-center gap-1 text-sm text-purple-600 dark:text-purple-400">
                <TrendingUp className="w-3 h-3" />
                <span>+{estimatedScore} points</span>
              </div>
            </div>
          </div>

          {/* Cognitive domains grid */}
          <div className="grid grid-cols-5 gap-2">
            {DOMAIN_ICONS.map(({ icon: Icon, label, color }) => (
              <div key={label} className="text-center">
                <div className="bg-white/50 dark:bg-white/5 rounded-lg p-2 mb-1">
                  <Icon className={`w-5 h-5 mx-auto ${color}`} />
                </div>
                <div className="text-[10px] text-gray-500 dark:text-gray-400 truncate">{label}</div>
              </div>
            ))}
          </div>

          {/* Fake progress bars */}
          <div className="mt-4 space-y-2">
            {[65, 78, 52, 89, 71].map((value) => (
              <div key={`progress-bar-${value}`} className="flex items-center gap-2">
                <div className="flex-1 bg-gray-200 dark:bg-neo-navy-elevated rounded-full h-2">
                  <div
                    className="bg-purple-500 h-2 rounded-full"
                    style={{ width: `${value}%` }}
                  />
                </div>
                <span className="text-xs text-gray-500 w-8">{value}</span>
              </div>
            ))}
          </div>
        </div>
      </div>
    </m.div>
  );
};

export default GuestBrainScorePreview;
