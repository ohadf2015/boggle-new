...
import { Flame, Hammer, Shuffle, Trophy, Users } from 'lucide-react';
...
/** Perfect drops per wrecking ball (run.ts BALL_EVERY_COMBO). */
const BALL_EVERY = 3;

/**
 * Word Tower v2 HUD: floor count first (it is an apartment tower), metres and
 * score under it, live crate effects as chips, and a combo meter whose pips
 * fill toward the next wrecking ball. Celebrations live in V2Celebrations.
 */
  ...
  return (
    <>
      <div className="pointer-events-none absolute start-3 top-3 z-20 flex flex-col items-start gap-1.5" aria-live="polite">
        <div
          className="flex items-end gap-2 rounded-neo border-neo-thick border-black bg-neo-lime px-3 py-1 text-neo-navy shadow-hard"
          aria-label={t('wordTowerV2.hud.floorA11y', { n: floors, m: heightM.toFixed(1) })}
        >
          <div className="flex flex-col leading-none" aria-hidden>
            <span className="font-neo-display text-[10px] font-black uppercase tracking-widest">{t('wordTowerV2.hud.floor')}</span>
            <span className="font-neo-display text-4xl font-black tabular-nums">{floors}</span>
          </div>
          <span className="mb-1 font-neo-display text-sm font-bold tabular-nums opacity-80" aria-hidden>
            {heightM.toFixed(1)}
            {t('wordTowerV2.unitM')}
          </span>
        </div>
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          <div
            ...
            aria-label={`${t('wordTower.hud.scramble')} ${run.scrambles}`}
          >
          ...
          {tenants > 0 ? (
            <div
              key={`tenants-${tenants}`}
              className="flex items-center gap-1 rounded-neo border-neo border-black bg-neo-cream px-2 py-0.5 font-neo-display text-xs font-bold text-neo-navy shadow-hard-sm animate-neo-pop"
          ...
          <div className="flex flex-wrap gap-1.5">
            {effects.map(({ id, n }) => {
              ...
              return (
                <div
                  key={`${id}-${n}`}
                  className={`flex items-center gap-1 rounded-neo border-neo border-black px-2 py-0.5 font-neo-display text-xs font-black text-neo-navy shadow-hard-sm animate-neo-pop ${EFFECT_CLASS[id]}`}
      ...
      {run.combo >= 2 ? (
        <div
          key={run.combo}
          className="pointer-events-none absolute left-1/2 top-3 z-20 flex -translate-x-1/2 flex-col items-center gap-1 animate-neo-pop"
          aria-label={t('wordTower.a11y.combo', { n: run.combo })}
        >
          <div className="flex items-center gap-1 rounded-neo border-neo-thick border-black bg-neo-orange px-3 py-1 font-neo-display text-lg font-black text-neo-navy shadow-hard">
            <Flame className="h-5 w-5" aria-hidden />×{run.combo}
          </div>
          <div className="flex gap-1" aria-hidden>
            {Array.from({ length: BALL_EVERY }, (_, i) => (
              <span
                key={i}
                className={`h-2 w-5 rounded-sm border-2 border-black ${i < run.combo % BALL_EVERY || run.combo % BALL_EVERY === 0 ? 'bg-neo-pink' : 'bg-neo-navy'}`}
              />
            ))}
          </div>
        </div>
