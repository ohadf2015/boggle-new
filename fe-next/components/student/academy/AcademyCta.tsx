 ...
 *   next lesson → "PLAY · <lesson>" · review due → "REVIEW · 5 missed words"
...
export function AcademyCta({ kind, activeGame, target, targetLabel, isJoining, joinError, onPress, reducedMotion }: Props) {
  const { t } = useLanguage();
  ...
  const title =
    kind === 'live'
      ...
      : kind === 'join-class'
        ...
        : kind === 'review'
          ? t('academy.student.ctaReviewTitle', '{count} missed words', { count: target?.badge ?? 0 })
          : kind === 'boss'
  ...
  const tone: Tone = kind === 'live' ? 'pink' : kind === 'join-class' || kind === 'solo' ? 'teal' : 'gold';
  ...
  return (
    <div className="relative">
      ...
      <m.button
        ...
        disabled={isJoining}
        className="relative flex h-[68px] w-full items-center gap-3 overflow-hidden rounded-[20px] border-3 border-neo-black ps-2 pe-2 text-start text-neo-black disabled:cursor-wait sm:h-[76px]"
      ...
      >
        ...
        <span className="relative min-w-0 flex-1">
          ...
          <span dir="auto" className="block truncate font-neo-display text-xl font-black uppercase leading-none tracking-tight sm:text-2xl">
            {title}
          </span>
          {sub && (
        ...
        <span className="relative flex shrink-0">
          <Medallion tone="night" size={46} shadow={2}>
            {isJoining ? (
