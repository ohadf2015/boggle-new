...
<task type="auto">
  <name>Task 1: Write failing tests for bulk approve endpoint</name>
  <files>app/api/admin/invalid-words/bulk-approve/__tests__/route.test.ts</files>
  <action>
Create test file with comprehensive tests (RED phase):
...
Note: This test file tests the business logic function directly, not the HTTP handler, following Phase 8's pattern for avoiding Next.js runtime issues in Jest.
  ...
  <verify>npm run test:frontend -- bulk-approve --passWithNoTests</verify>
  <done>Test file exists with comprehensive test cases covering validation, approval logic, and error handling</done>
</task>

<task type="auto">
  <name>Task 2: Implement bulk approve endpoint to pass tests</name>
  <files>app/api/admin/invalid-words/bulk-approve/route.ts</files>
  <action>
Create the API route implementation (GREEN phase):
