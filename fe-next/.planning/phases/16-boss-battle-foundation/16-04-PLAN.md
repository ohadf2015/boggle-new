...
must_haves:
  truths:
    - "World 1 boss battle uses popQuiz mechanic to challenge player with rotating word requirements"
    - "popQuiz multipliers integrate with boss HP damage calculations (1.5x bonus, 0.8x penalty)"
    - "popQuiz feedback displays during boss battle to inform player of success/failure"
  artifacts:
    - path: "hooks/__tests__/useBossMechanics.popQuiz.test.ts"
      provides: "Comprehensive popQuiz mechanic tests for boss battle integration"
      min_lines: 80
  key_links:
    ...
    - from: "hooks/useBossMechanics.ts"
      to: "components/adventure/AdventureGame.tsx"
      via: "checkBossWord returns scoreMultiplier for damage calculation"
      pattern: "checkBossWord"
---

<objective>
Verify popQuiz mechanic integration with World 1 boss battle.

Purpose: The popQuiz mechanic (BOSS-03) is Ms. Grammar's unique boss twist in World 1. This plan verifies that popQuiz correctly:
1. Integrates with boss HP damage calculations (multipliers affect damage dealt)
2. Provides player feedback during active boss battles
3. Rotates requirements to create dynamic boss fight experience

Output: Complete test coverage proving popQuiz works within the boss battle system
</objective>
