...
- **Open issues:**
  ...
  - **Unverified/not yet covered:** visual QA (not captured this run — time budget spent on full code audit of an 846-line untouched file + 3 backend modules), a11y deep pass (keyboard-only wheel input, color-contrast on fog-masked chips), perf/memo audit of `WheelRushView.tsx`'s render tree, Hebrew/Japanese native-speaker translation review, disconnect-mid-round + refresh-mid-round manual verification.
