...
**Update 2026-08-27:** `/en/multiplayer` p75 LCP **2141ms, n=362** (7d, clears floor) — now GOOD, first clean reading above n=20. `/es/multiplayer` p75 LCP **3378ms, n=154** (7d, clears floor) — still POOR-adjacent, ES lags EN by ~1200ms. No comparable same-route baseline exists in this file to call a delta; recording as new floor-clearing datapoint. Root cause below (game client / socket handshake as LCP element) still untested — checked `PageClient.tsx` (692 lines) tonight: no `<img>`/`next/image` on the page, confirming LCP element is DOM/component-driven not image-driven, consistent with the socket-handshake theory. Also correlates with the open `MP CLS 0.92+ (socket connecting→lobby DOM swap)` item in memory — likely same root DOM swap drives both LCP and CLS. Did not attempt the SSR-skeleton fix tonight (692-line live game client, needs the 4-6h scoped fix already logged, not a same-night blind edit near time budget). Next lane: build the lobby-waiting skeleton described below, prioritize ES since it's furthest from goal.  
...
## [FRONTEND] `/en/student/profile` — fully client-rendered, fetch waterfall before paint — 2026-09-15

**Severity:** Low-medium (noindex authed page, n=1/24h — cannot verdict as regression, this is a code-inspection finding, not a measured one)
**Source:** brief flagged 9184ms LCP on this route; PostHog re-query showed n=1, below the 50-sample floor — no regression verdict possible.
