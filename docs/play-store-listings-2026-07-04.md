# Play Store Listings — LIVE COPY (source of truth)

> Updated 2026-07-04 via androidpublisher API (edit committed; managed publishing OFF — changes auto-publish once Google review approves, verified in Console same day).
> Changes: NEW ru-RU listing · 5→6 languages everywhere · Word Tower added to mode lists · removed Jackbox/Boggle trademark line (Play metadata policy risk).
> Supersedes: docs/google-play-store-listing.md, docs/store-listing-hebrew.md, docs/store-listing-swedish.md, docs/store-listing-japanese.md (copy sections only — those files keep tags/rating-questionnaire notes).
> Screenshots: ru-RU / es-419 / es-ES each got 6 localized phone screenshots (uploaded via API same day; sources in screenshots/store-final/{ru,es}/phone/, regenerate with captioned-promo pipeline: raw capture of prod at 360×640@3x with onboarding+consent localStorage pre-seeded, then HTML compose at 540×960@2x).

## en-GB

**Title** (26/30): LexiClash: Word Party Game

**Short** (77/80): Live multiplayer word games: swipe letters, build words, battle friends fast.

**Video**: https://www.youtube.com/watch?v=RNY82eo66RY

**Full description**:

```
Real-time multiplayer word games. Build words, battle friends live, and climb global leaderboards — no waiting for turns.

You know that feeling when you spot a word nobody else saw? That rush is what LexiClash is built on. Swipe letters, build words, outscore everyone — all happening live. Real opponents, real pressure, real satisfaction when you drop a 7-letter bomb while everyone else is stuck on CAT.

WHY IT'S DIFFERENT
Most word games are slow — play a word, put the phone down, wait for grandma to respond next Tuesday. LexiClash runs in real time. Everyone plays the same board, same clock, same panic. A live word hunt where the fastest mind wins.

6 LANGUAGES, REAL DICTIONARIES
Play in English, Spanish, Russian, Hebrew, Swedish, or Japanese — not just translated menus, but full word dictionaries for each language. Switch mid-session whenever you want.

GAME MODES
• Word Arena — live multiplayer word battles. Quick Start finds an opponent in seconds, or make a private room for people you know.
• Daily Challenge — a fresh daily word challenge. Same puzzle for everyone; compare your score against the whole player base.
• Adventure — 100+ levels across themed worlds with new rules each world: gravity, locked tiles, and other curveballs.
• Blast — fast tile-clearing combo chains. Chain words, trigger cascades, beat the board.
• Word Tower — turn words into floors and stack the tallest tower you can.
• Word Bridge — chain-building logic puzzles that connect words into a path.
• Brain Training — five cognitive drills (memory, pattern, combo, rare words, lightning round) to sharpen your game.
• WordCraft — claim territory by building words across the board, tile by tile.
• Practice — solo, no timer. Warm up or just pretend you're being productive.

YOUR PROGRESS
Global leaderboards with tier rankings (Bronze to Diamond). Track best words, longest streaks, highest scores. Compete for #1 or just beat your roommate.

THE DESIGN
Dark theme, bold colors, thick borders, zero gradients. Neo-brutalist — call it whatever you want, it doesn't look like every other word game.

NO PAYWALLS
Play the full game without spending. Optional cosmetics if you want to look fancy; the game works without them.

Download, find a word, talk trash to your friends.
```

## iw-IL

**Title** (21/30): LexiClash: משחק מילים

**Short** (72/80): משחק מילים רב-משתתפים בזמן אמת — בנו מילים, התחרו בחברים ונצחו את השעון.

**Full description**:

```
משחק מילים רב-משתתפים בזמן אמת. בנו מילים, התחרו בחברים ותטפסו בלוחות התוצאות — בלי לחכות לתור.

החליקו על האותיות, בנו מילים והשיגו ניקוד גבוה מכולם — הכול קורה בשידור חי, מול יריבים אמיתיים.

6 שפות, מילונים אמיתיים
שחקו בעברית, אנגלית, רוסית, שוודית, יפנית או ספרדית — לא רק תפריט מתורגם, אלא מילון מלא לכל שפה.

מצבי משחק
• זירת מילים — קרבות מילים חיים מול שחקנים אמיתיים או בוטים.
• אתגר יומי — אותו פאזל לכולם, כל יום.
• הרפתקה — 100+ שלבים בעולמות עם חוקים משתנים.
• Blast — שרשראות קומבו לניקוי משבצות.
• מגדל מילים — הופכים מילים לקומות ובונים את המגדל הגבוה ביותר.
• גשר מילים — חידות היגיון של חיבור מילים.
• אימון מוח — חמישה תרגילים קוגניטיביים.
• WordCraft — כיבוש טריטוריה דרך בניית מילים.
• תרגול — מצב יחיד, בלי לחץ זמן.

עיצוב נאו-ברוטליסטי: רקע כהה, צבעים עזים, מסגרות עבות. בלי חומות תשלום — כל המשחק פתוח.
```

## sv-SE

**Title** (18/30): LexiClash: Ordspel

**Short** (75/80): Ordspel för flera spelare i realtid – bygg ord, utmana vänner, slå klockan.

**Full description**:

```
Ordspel för flera spelare i realtid. Bygg ord, utmana vänner och klättra på topplistorna – utan att vänta på din tur.

Svep över bokstäverna, bygg ord och pressa alla andra – allt händer live, mot riktiga motståndare.

6 SPRÅK, RIKTIGA ORDLISTOR
Spela på svenska, engelska, ryska, hebreiska, japanska eller spanska – inte bara översatta menyer, utan en komplett ordlista för varje språk.

SPELLÄGEN
• Ordarena – live ordstrider mot riktiga spelare eller bottar.
• Daglig utmaning – samma pussel för alla, varje dag.
• Äventyr – 100+ nivåer med nya regler i varje värld.
• Blast – snabba kombokedjor som rensar brickor.
• Ordtornet – gör ord till våningar och bygg det högsta tornet.
• Ordbro – logikpussel där du länkar ord.
• Hjärnträning – fem kognitiva övningar.
• WordCraft – erövra territorium genom att bygga ord.
• Träning – ensam, utan tidspress.

Neo-brutalistisk design: mörkt tema, djärva färger, tjocka kanter. Inga betalväggar – hela spelet är öppet.
```

## ja-JP

**Title** (16/30): LexiClash: 単語ゲーム

**Short** (37/80): リアルタイム対戦の単語ゲーム。文字をなぞって単語を作り、友達と勝負しよう。

**Full description**:

```
リアルタイム対戦の単語ゲーム。文字をつないで単語を作り、友達と競い合おう——順番待ちはなし。

文字をなぞって単語を作り、ライバルより高いスコアを狙う。すべてがライブで進行します。

6言語、本格辞書搭載
日本語・英語・ロシア語・ヘブライ語・スウェーデン語・スペイン語でプレイ。メニューだけでなく、各言語に本物の辞書を搭載。

ゲームモード
• ワードアリーナ — リアルタイムの対戦バトル。
• デイリーチャレンジ — 毎日、全員同じパズル。
• アドベンチャー — ルールが変わる100以上のステージ。
• Blast — タイルを消すコンボチェイン。
• ワードタワー — 単語をフロアに変えて、最も高いタワーを目指そう。
• ワードブリッジ — 単語をつなぐ論理パズル。
• 脳トレ — 5種類の認知トレーニング。
• WordCraft — 単語で陣地を広げる。
• 練習 — 時間制限なしのソロモード。

ネオブルータリズムのデザイン。ダークテーマ、大胆な色、太い枠線。課金の壁なし——全モードが無料で遊べます。
```

## es-419

**Title** (28/30): LexiClash: Juego de palabras

**Short** (78/80): Juego de palabras multijugador en tiempo real: forma palabras y reta a amigos.

**Full description**:

```
Juego de palabras multijugador en tiempo real. Forma palabras, reta a tus amigos y escala en las clasificaciones, sin esperar turnos.

Desliza las letras, forma palabras y supera a todos: todo sucede en vivo, contra rivales reales.

6 IDIOMAS, DICCIONARIOS REALES
Juega en español, inglés, ruso, hebreo, sueco o japonés: no solo menús traducidos, sino un diccionario completo para cada idioma.

MODOS DE JUEGO
• Arena de palabras — batallas en vivo contra jugadores reales o bots.
• Reto diario — el mismo puzle para todos, cada día.
• Aventura — más de 100 niveles con reglas nuevas en cada mundo.
• Blast — cadenas de combos para limpiar fichas.
• Torre de palabras — convierte palabras en pisos y construye la torre más alta.
• Puente de palabras — puzles de lógica que enlazan palabras.
• Entrenamiento mental — cinco ejercicios cognitivos.
• WordCraft — conquista territorio formando palabras.
• Práctica — modo en solitario, sin presión de tiempo.

Diseño neo-brutalista: tema oscuro, colores intensos, bordes gruesos. Sin muros de pago: el juego completo está abierto.
```

## es-ES

**Title** (28/30): LexiClash: Juego de palabras

**Short** (78/80): Juego de palabras multijugador en tiempo real: forma palabras y reta a amigos.

**Full description**:

```
Juego de palabras multijugador en tiempo real. Forma palabras, reta a tus amigos y escala en las clasificaciones, sin esperar turnos.

Desliza las letras, forma palabras y supera a todos: todo sucede en vivo, contra rivales reales.

6 IDIOMAS, DICCIONARIOS REALES
Juega en español, inglés, ruso, hebreo, sueco o japonés: no solo menús traducidos, sino un diccionario completo para cada idioma.

MODOS DE JUEGO
• Arena de palabras — batallas en vivo contra jugadores reales o bots.
• Reto diario — el mismo puzle para todos, cada día.
• Aventura — más de 100 niveles con reglas nuevas en cada mundo.
• Blast — cadenas de combos para limpiar fichas.
• Torre de palabras — convierte palabras en pisos y construye la torre más alta.
• Puente de palabras — puzles de lógica que enlazan palabras.
• Entrenamiento mental — cinco ejercicios cognitivos.
• WordCraft — conquista territorio formando palabras.
• Práctica — modo en solitario, sin presión de tiempo.

Diseño neo-brutalista: tema oscuro, colores intensos, bordes gruesos. Sin muros de pago: el juego completo está abierto.
```

## ru-RU

**Title** (23/30): LexiClash: Игра в слова

**Short** (78/80): Игра в слова онлайн: составляйте слова и побеждайте друзей в реальном времени.

**Full description**:

```
Многопользовательская игра в слова в реальном времени. Составляйте слова, соревнуйтесь с друзьями и поднимайтесь в таблице лидеров — без ожидания хода.

Проводите пальцем по буквам, собирайте слова и набирайте больше очков, чем соперники, — всё происходит вживую, против настоящих игроков.

6 ЯЗЫКОВ, НАСТОЯЩИЕ СЛОВАРИ
Играйте на русском, английском, испанском, иврите, шведском или японском — не просто переведённое меню, а полноценный словарь для каждого языка.

РЕЖИМЫ ИГРЫ
• Арена слов — живые словесные баталии с реальными игроками и ботами.
• Ежедневный вызов — одна головоломка для всех, каждый день.
• Приключение — 100+ уровней с новыми правилами в каждом мире.
• Blast — цепочки комбо, очищающие поле.
• Башня слов — превращайте слова в этажи и стройте самую высокую башню.
• Мост слов — логические головоломки, соединяющие слова.
• Тренировка мозга — пять когнитивных упражнений.
• WordCraft — захватывайте территорию, составляя слова.
• Практика — одиночный режим без таймера.

Нео-бруталистский дизайн: тёмная тема, яркие цвета, толстые рамки. Без стен оплаты — вся игра открыта.
```

