...
### Production hardening — all JA validation + board surfaces unified to hiragana (2026-05-21, follow-up commit)
After the MP fix shipped, an audit of *every* JA dictionary + board generator found the kanji/hiragana split was wider than MP — and that flipping backend validation to hiragana had **exposed a latent score-loss regression**: SP/daily/offline word scores were re-validated at `/api/scores/sync` against a kanji dictionary, silently rejecting valid hiragana words. All surfaces are now consistent:
- **`shared/constants/japaneseLetters.ts`** — new single source of truth (frequency-weighted hiragana pool + `isHiraganaWord` + `extractHiraganaWords`). Backend grid, frontend grid, both validation routes, and the dict-asset build all import it, so the board and the dictionary can never drift again.
- **`lib/wordValidation/serverDicts.ts`** — `loadJapanese` now reads `japanese_words.txt` (hiragana) not `kanji_compounds.txt`. Fixes the `/api/scores/sync` rejection of valid hiragana words.
- **`app/api/dictionary-words/route.ts`** — ja branch serves hiragana (feeds the client `useDictionaryCache`/`fastValidateWord`).
- **`scripts/build-dict-assets.ts`** — `ja.dict.gz` (offline/word-solver/adventure client validation) now built from hiragana; 9,667 words, 27.7 KB. (`public/dicts/` is gitignored → the deploy rebuilds it from this script.)
- **`utils/consts.ts`** — `japaneseLetters`/`kanjiCompounds` are now hiragana, fixing **all five** frontend JA board generators at the root: single-player, custom challenge, daily challenge, brain drills, adventure.
...
- **Dead kanji constants** (`backend/utils/gameUtils.ts:61-83` `japaneseLetters`/`kanjiCompounds`) are now unused but left exported to avoid churn; safe to delete in a cleanup pass.
