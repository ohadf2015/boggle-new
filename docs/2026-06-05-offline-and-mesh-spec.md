...
- **Connections (SHIPPED):** mirror of blast — extracted `processConnectionsCompletion` from `/api/connections/daily/score` (4 route tests preserved; identity-parameterized so guest + authed callers share it; naturally idempotent via best-keeps upsert), `dispatchConnections` in the sync route (admin client, validates via `validateDailySubmission`), `'connections'` in the 3 enums, and client enqueue in the LIVE path `submitDailyScore` (`dailyClient.ts`) — offline + cached session → enqueue `'connections'`; guests (no session) fall through. (An orphan `saveConnectionsResult.ts` from the first pass was removed in favor of wiring the real submit path.)

**A1.2 status: COMPLETE.** Phase 0 + Blast + Connections all shipped; sp/wotd/daily-* remain on the existing word-revalidation path (no processCompletion yet — pre-existing Phase 1c).
