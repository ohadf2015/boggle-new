...
| TD-006 | Code Quality | 226 `any` / `as any` — **partial**: `supabaseRealtime.ts` 13→0 via SDK generics + narrowed row types. Remaining: `blastJuiceKit.ts` (13), `app/api/admin/game-logs/route.ts` (12) + ~50 files | `lib/supabaseRealtime.ts` (done), `blastJuiceKit.ts`, `app/api/admin/game-logs/route.ts` | L | High | 3.0 | 2026-04-22 | In progress | External API typing friction |
