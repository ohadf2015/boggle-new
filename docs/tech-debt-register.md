...
|----|----------|-------------|-------|--------|--------|----------|-------|--------|--------------|
| TD-001 | Architecture | 20+ rune abilities stubbed as `TODO` no-ops — rune engine silently broken for edge/gold tiles, boss multiplier, timer freeze | `lib/wordForge/runeEngine.ts:223–651` (683 lines) | L | Critical | 4.0 | 2026-04-22 | Backlog | Shipped engine without full rune coverage; users hit inert abilities |
| TD-002 | Architecture | `GameStateContext` marked `@deprecated` but still in provider tree — every consumer re-renders on unrelated state changes | `contexts/GameStateContext.tsx:67,84` | M | High | 3.0 | 2026-04-22 | Backlog | Zustand migration mid-flight |
