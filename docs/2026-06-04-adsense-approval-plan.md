...
- [ ] Phase 1.3 — (deferred) evaluate noindex for EN-only anagram per-letter pages (already EN-only-gated, lower priority).
- [ ] Phase 2.6 — (optional) dynamic "latest blog posts" feed on homepage (static editorial links already shipped in 2.5).
- [ ] Phase 3 — soft-404 remediation (inspect the 135 soft-404 URLs in GSC).
- [ ] Phase 4 — resubmit trimmed sitemap in GSC + reapply after indexed count drops.

### Status after this session
Phase 1 (the dominant lever — cull the thin indexed surface) + the highest-ROI half of Phase 2 (un-hide the homepage prose) are implemented and tested (10 tests green, tsc + lint clean). Remaining work is lower-leverage cleanup (1.3, 2.6, 3) and the operational reapply protocol (Phase 4), which is gated on Google re-crawling the now-noindexed pages (~1–3 weeks).
